...
# External functions
from fem_gpe.norms import l2_norm, scaled_h1_norm
...
k_values = config["mesh"]["resolution"]["k_values"]
degree = config["mesh"]["element"]["degree"]

if not isinstance(k_values, list) or len(k_values) == 0:
    raise ValueError("mesh.resolution.k_values must be a non-empty list.")
...
for k in k_values:
    ...
    ### H1 and H1_eps norms of the ground state
    h1_phi00_scaled = scaled_h1_norm(epsilon, phi_00, domain, quadrature_degree=3)
    h1_phi00_unscaled = scaled_h1_norm(1, phi_00, domain, quadrature_degree=3)



    if rank == 0:
        print(f"H1-eps: {h1_phi00_scaled}")
        print(f"H1: {h1_phi00_unscaled}")
    ...
    comm=comm,
    rank=rank,
    h1_scaled=h1_phi00_scaled,
    h1_unscaled=h1_phi00_unscaled,)
